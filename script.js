...
function render(goals) {
  ...
  Object.entries(goals).forEach(([category, items]) => {
    ...
    items.forEach((item, idx) => {
      ...
      ["Bassa", "Media", "Alta"].forEach(level => {
        const opt = document.createElement("option");
        opt.value = level;
        opt.textContent = level;
        if (item.priority === level) opt.selected = true;
      ...
      item.sub.forEach((subItem, sIdx) => {
        const subLi = document.createElement("li");
        const subInput = document.createElement("input");
        subInput.type = "text";
        subInput.value = subItem;
        subInput.onchange = async () => {
          item.sub[sIdx] = subInput.value;
          await saveGoals(goals);
        };
        const delSub = document.createElement("button");
        delSub.textContent = "❌";
        delSub.onclick = async () => {
          item.sub.splice(sIdx, 1);
          await saveGoals(goals);
          init();
        };
      ...
      addSub.onkeypress = async (e) => {
        if (e.key === "Enter" && addSub.value.trim()) {
          item.sub.push(addSub.value.trim());
          await saveGoals(goals);
          init();
        }
      };
    ...
    input.onkeypress = async (e) => {
      if (e.key === "Enter" && input.value.trim()) {
        goals[category].push({ text: input.value.trim(), done: false, sub: [] });
        await saveGoals(goals);
        init();
      }
    };
